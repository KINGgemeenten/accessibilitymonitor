...
class PhantomQuailWorker extends \Thread
{
    ...
    public function run()
    {
        try {
            ...
            $this->log(LogLevel::EMERGENCY,
              sprintf('Uncaught exception: %s.', $e->getMessage()));
        }
    }
    ...
    protected function detectCms()
    {
        ...
        $this->websiteCms = implode('|',
          $this->phantomJs->getDetectedApps($testUrl));
    }
        ...
        if (count($this->quailCases)) {
            ...
            try {
                ...
                $this->log(LogLevel::ERROR,
                  'Error sending cases to solr. Solr responded with an exception: ' . $e->getMessage());
            }
        }
    }
