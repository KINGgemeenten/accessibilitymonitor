...
class Storage implements StorageInterface {
   ...
   */
  public function __construct(DatabaseInterface $database, Client $solrClient, LoggerInterface $logger) {
  ...
  public function getUrlById($id) {
    $query = $this->database->getConnection()->prepare("SELECT * FROM url WHERE url_id = :url_id");
    $query->execute(array(
      'url_id' => $id,
    ));
  ...
  public function getUrlsByWebsiteTestResultsId($website_test_results_id) {
    $query = $this->database->getConnection()->prepare("SELECT * FROM url WHERE website_test_results_id = :website_test_results_id");
    $query->execute(array(
  ...
  public function getUrlsByNotStatus($status) {
    $query = $this->database->getConnection()->prepare("SELECT * FROM url WHERE cms IS NULL AND status != :status");
    $query->execute(array(
      'status' => $status
    ));
    $urls = array();
  ...
  public function countUrlsByWebsiteTestResultsId($website_test_results_id) {
    $query = $this->database->getConnection()->prepare("SELECT count(*) FROM url WHERE website_test_results_id = :website_test_results_id");
    $query->execute(array(
      'website_test_results_id' => $website_test_results_id,
    ));
  ...
  public function countUrlsByWebsiteTestResultsIdAndUrl($website_test_results_id, $url) {
    $query = $this->database->getConnection()->prepare("SELECT count(*) FROM url WHERE website_test_results_id = :website_test_results_id AND url = :url");
    $query->execute(array(
      'url' => $url,
      'website_test_results_id' => $website_test_results_id,
    ));
  ...
  public function countUrlsByStatusAndWebsiteId($status, $website_test_results_id) {
    $query = $this->database->getConnection()->prepare("SELECT count(*) FROM url WHERE website_test_results_id = :website_test_results_id AND status = :status");
    $query->execute(array(
      'website_test_results_id' => $website_test_results_id,
      'status' => $status,
    ));
  ...
  public function saveUrl(Url $url) {
    ...
    if ($url->getId()) {
      ...
      $query = $this->database->getConnection()->prepare("UPDATE url SET status = :status, cms = :cms, quail_result = :quail_result, pagespeed_result = :pagespeed_result, priority = :priority, analysis = :analysis, is_root = :is_root WHERE url_id = :url_id");
      $query->execute($values);
    }
    else {
      $values['url'] = $url->getUrl();
      $values['website_test_results_id'] = $url->getWebsiteTestResultsId();
      $insert = $this->database->getConnection()->prepare("INSERT INTO url (website_test_results_id, url, status, priority, cms, quail_result, pagespeed_result, analysis, is_root) VALUES (:website_test_results_id, :url, :status, :priority, :cms, :quail_result, :pagespeed_result, :analysis, :is_root)");
  ...
  public function countCmsTestResultsByWebsiteTestResultsId($website_test_results_id) {
    $query = $this->database->getConnection()->prepare("SELECT COUNT(*) FROM url WHERE website_test_results_id = :website_test_results_id AND cms IS NOT NULL");
    $query->execute(array(
      'website_test_results_id' => $website_test_results_id,
    ));
  ...
  public function countGooglePagespeedResultsByWebsiteTestResultsId($website_test_results_id) {
    $query = $this->database->getConnection()->prepare("SELECT COUNT(*) FROM url WHERE website_test_results_id = :website_test_results_id AND pagespeed_result <> ''");
    $query->execute(array(
      'website_test_results_id' => $website_test_results_id,
    ));
  ...
  public function getUrlLastAnalysisDateTime() {
    $query = $this->database->getConnection()->prepare("SELECT analysis FROM url WHERE ORDER BY analysis DESC LIMIT 1");
    $query->execute();
     ...
     *   Solr document
     */
    protected function caseToSolrDocument(Url $url, Query $updateQuery, \stdClass $case) {
        ...
        if (property_exists($case, 'uniqueKey')) {
            ...
            $doc->setField('url_id', $this->escapeStringForSolr($url->getUrl()));
            ...
            $doc->setField('website_test_results_id', $url->getWebsiteTestResultsId());
